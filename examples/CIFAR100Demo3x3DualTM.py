...
f = open("cifar100_%.1f_%d_%d_%d_%.2f_%d_%d_%.2f.txt" % (
s, clauses, T, patch_size, literal_drop_p, resolution, max_included_literals, q), "w+")
for ensemble in range(ensembles):
    tm_positive = MultiClassConvolutionalTsetlinMachine2D(clauses, T, s, (32, 32, 3*resolution), (patch_size, patch_size), max_included_literals=max_included_literals, q=1)
    tm_negative = MultiClassConvolutionalTsetlinMachine2D(clauses, T, s, (32, 32, 3*resolution), (patch_size, patch_size), max_included_literals=max_included_literals, q=-1)
